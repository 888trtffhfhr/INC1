...
public class MainListAdapter extends BaseAdapter {
	...
	private List<MediaCard> mediaCards;
	private Context context;

	public MainListAdapter(Context context, BitmapUtils bitmapUtils) {
		this.context = context;
		this.bitmapUtils = bitmapUtils;
	}

	public void setDatas(List<MediaCard> mediaCards) {
	...
	public View getView(int position, View convertView, ViewGroup arg2) {
		...
		MediaCard mediaCard = mediaCards.get(position);
		...
		final String nextURL = mediaCard.getNextUrl();

		final MediaCard valueCard = mediaCard;
		...
		viewHolder.imageview.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				intent.putExtra("value", valueCard);
		...
		if (likes_show.size() <= 7) {
			for (int i = 0; i < likes_show.size() - 1; i++) {
				int index = i + 1;
				bitmapUtils.display(images[i], likes_show.get(index));
			}
		} else {
			for (int i = 0; i < 7; i++) {
				int index = i + 1;
